...
AVLTreeNode* findHelper(AVLTreeNode *inSubTree, string valToFind)
{
    if (!inSubTree)
        return nullptr;

    if (inSubTree->data().name() == valToFind)
        return inSubTree;
...
AVLTreeNode*
AVLTree:: find(std::string valToFind) const
{
...
AVLTreeNode* insertHelper(AVLTreeNode *intoSubTree, Person valToAdd)
{
    ...
    if (valToAdd.name() < intoSubTree->data().name() )
    {
        ...
        if (leftHeight-rightHeight==2) 
	{
	  ...
	  if (valToAdd.name() < intoSubTree->left()->data().name() )
	  {
    ...
    else if (valToAdd.name() > intoSubTree->data().name() )
    {
        ...
        if (rightHeight-leftHeight==2) 
	{
	  ...
	  if (valToAdd.name() >  intoSubTree->right()->data().name() )
	    {
...
AVLTreeNode*
removeHelper(Person existingVal, AVLTreeNode *fromSubTree)
{
...
void
AVLTree::remove(std::string existingVal)
{
  AVLTreeNode *toDel = find(existingVal);
    ...
    _root=removeHelper(toDel->data(), _root);
}
