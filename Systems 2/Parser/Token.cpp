...
void Token::get(istream &is)
{
  ...
  is.clear(); //clears errors(getting to the end of the file previously, if seek is used, I still need to be able to read the next token. If not, it either already returned or will return eof_tok next time depending on circumstances
}
